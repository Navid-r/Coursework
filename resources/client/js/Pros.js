...
function getProsList() {
    ...
    fetch(url, {
        ...
        if (response.hasOwnProperty("Error")) { //checks if response from the web server has an "Error"
            ...
            formatProsList(response);          //this function will create an HTML table of the data
        }
    });
}

function formatProsList(myJSONArray) {
    if (myJSONArray[0] == undefined) { //if the ProName does not exist in the case of inputting non existant names
        ...
        for (let item of myJSONArray) {
            dataHTML += "<tr>" + "<td>" + item.ProName + "</td>" + "<td>" + item.Position + "</td>" + "<td>" + item.AverageKills + "</td>" + "<td>" + item.AverageDeaths + "</td>" + "<td>" + item.AverageAssists + "</td>" + "<td>" + item.KDA + "</td>" + "</tr>";//formatting of the table
        }
...
function getPros(ProName) { //fetches the a particular pro
    debugger;
...
function valPros(){  //function for validating user inputs for ProName
    ...
    if (formData.get('ProName').length < 2 || formData.get('ProName').length >15){
        ...
        getPros(formData.get('ProName'))//calls getPros
    }
}